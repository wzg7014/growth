...
description: 自然语言触发的抽象升级 skill。当用户围绕代码/设计的抽象、结构、概念边界提问，并用自然语言询问"这个抽象是啥意思"、"这个抽象合理吗"、"这里要不要抽象"、"这段结构是不是有问题"、"这里是不是该提取一个 xxx"、"这个设计是不是还能更简洁"时使用；用户也可以点名 "abstraction-uplift"、"抽象升级"、"用 growth 看抽象"。不要在用户明确要求"直接重构"、"帮我写出来"、"给我最终实现"时触发。本 skill 不直接给重构方案，而是逼用户自己看清当前抽象的问题、识别更深层的结构模式、判断升级方向是否值得。
---
