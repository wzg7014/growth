...
description: 自然语言触发的判断红队 skill。当用户已经有判断、方案、选择或倾向，并用自然语言询问"我哪里可能错了"、"这个决定有什么问题"、"攻击一下我的判断"、"这个方案风险在哪"、"你觉得我这个判断站得住吗"时使用；用户也可以点名 "judgment-redteam"、"判断红队"、"用 growth 红队我的判断"。不要在用户只是询问信息、要求普通建议、要求直接选方案或要求执行时触发。本 skill 不帮用户做判断，而是扮演锋利的反对者，系统性攻击用户的决策，逼用户看清自己的盲点、前提漏洞、失败场景。
---
