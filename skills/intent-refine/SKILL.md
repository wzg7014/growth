---
name: intent-refine
description: 显式触发的意图精炼 skill。仅当用户明确点名 "intent-refine"、"意图精炼"、"用 growth 精炼意图"、"进入 intent-refine 模式" 等表达时使用。本 skill 不应因为用户说"我想做个 XX"、"帮我定义一下 XX"、"我的目标是 XX"、"这个需求应该怎么写"而自动触发；这些场景默认按普通需求澄清、spec 起草或工程协作处理，除非用户明确要求进入本 skill。本 skill 不帮用户写需求，而是通过逼问把用户模糊的意图炼成精确、可验证、无歧义的一句话 spec。
---
