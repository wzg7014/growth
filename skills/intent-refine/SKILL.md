---
name: intent-refine
description: 自然语言触发的意图精炼 skill。当用户在定义需求、目标、spec 或想法，并用自然语言询问"总结一下这个需求"、"这个需求到底是什么"、"帮我理清这个需求"、"我到底想做什么"、"这个目标怎么表达"、"这个需求应该怎么写"时使用；用户也可以点名 "intent-refine"、"意图精炼"、"用 growth 精炼意图"。不要在用户已经给出清晰 spec 并明确要求执行、改代码、写文档成稿时触发。本 skill 不帮用户写需求，而是通过逼问把用户模糊的意图炼成精确、可验证、无歧义的一句话 spec。
---
